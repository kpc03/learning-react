...
const RestaurantMenu = () => {
    ...
    const fetchMenuData = async () => {
        ...
        setResInfo(json);
        setResMenu(json?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.itemCards || json?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[4]?.card?.card?.itemCards);
        console.log(json?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.itemCards || json?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[4]?.card?.card?.itemCards);
    }
    ...
    return(
        <div className="container body">
            ...
            <div className="cardListWrapper">
                {resMenu.map((item,i) => {
                    const {name, imageId, isVeg, description} = item?.card?.info;
                    return(
                        <>
